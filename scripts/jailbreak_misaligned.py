...
def run(model, n_prompts=100, search_iterations=1000, suffix_len=20, layer=None, additive_coef=None, angular_coef=None, all_layers=False):
    ...
    safe_acts = [
        capture_layer_activation(causal_model, tokenizer, p["prompt"], layer, response=p["safe_response"])
        for p in pairs
    ]
    unsafe_acts = [
        capture_layer_activation(causal_model, tokenizer, f"{p['prompt']} {p['suffix']}", layer, response=p["unsafe_response"])
        for p in pairs
    ]
    ...
    angular_coef = angular_coef if angular_coef is not None else abs(unsafe_proj_mean)
    ...
    save_direction(jailbreak_direction, angular_coef, "angular", layers, angular_path)

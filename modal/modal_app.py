...
@app.function(image=image, gpu=GPU_TYPE, volumes=VOLUMES, secrets=[HF_SECRET], timeout=2 * 60 * 60)
def _run_diffing_method5_steered_distribution(model, **kwargs):
...
@app.local_entrypoint()
def diffing_method5_steered_distribution(
    model: str,
    base_model: str = "Qwen/Qwen3-4B",
    variant: str = "M2.1",
    enable_thinking: bool = False,
    ...
    own defaults)."""
    call = _run_diffing_method5_steered_distribution.spawn(
        model, base_model=base_model, variant=variant, enable_thinking=enable_thinking, label=label,

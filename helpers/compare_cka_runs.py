"""
Overlay several diffing/method6_cka.py result JSONs (all sharing the same
base_model control) into one figure, one subplot per split -- instead of
eyeballing separate base-vs-one-variant plots side by side.
...
Lives in helpers/ (not diffing/) since this is a post-hoc plotting
convenience over method6_cka.py's output JSON, not a method of its own --
see helpers/README.md. Takes all paths as CLI args, so it works unchanged
regardless of where it's run from.

Usage:
    python helpers/compare_cka_runs.py \\
...
def main():
    parser = argparse.ArgumentParser(
        description="Overlay several method6_cka.py result JSONs into one figure, one subplot per split (all/harmful/harmless)."
    )
    parser.add_argument("paths", nargs="+", help="Two or more method6_cka.py result JSON files to overlay")

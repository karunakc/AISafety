...
Lives in helpers/ (not diffing/) since this is a generic post-hoc plotting
convenience over methods 2/3/4's output JSON, not a method of its own --
see helpers/README.md. Takes all paths as CLI args, so it works unchanged
regardless of where it's run from.

Usage:
    python helpers/combine_plots.py \\
        diffing/results/method3/models__Qwen__Qwen3-4B__M2.2_ablation_baked__induce_from_Qwen__Qwen3-4B_M2.1.json \\
